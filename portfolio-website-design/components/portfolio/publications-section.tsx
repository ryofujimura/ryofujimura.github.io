...
export function PublicationsSection() {
  return (
    <section
      id=\"publications\"
      className=\"relative py-20 sm:py-24 md:py-32 lg:py-40 px-4 sm:px-6 overflow-hidden bg-background border-t border-b border-foreground/10\"
    >
      ...
      />
      <div className=\"pointer-events-none absolute -top-6 left-6 h-16 w-16 border border-foreground/40\" aria-hidden />
      <div className=\"pointer-events-none absolute bottom-10 right-6 h-10 w-10 border border-dashed border-foreground/30\" aria-hidden />

      {/* Technical SVG field – draws in with GSAPSVG */}
      <GSAPSVG className=\"pointer-events-none absolute inset-y-16 right-[-10%] hidden xl:block w-[360px] text-foreground/40\">
      ...
      <div className=\"relative z-10 max-w-6xl mx-auto\">
        {/* Header */}
        <div className=\"mb-12 sm:mb-16 md:mb-20 flex flex-col md:flex-row md:items-end gap-6 md:gap-10\">
          <div className=\"space-y-3 sm:space-y-4 max-w-xl\">
            ...
            <GSAPText
              variant=\"chars\"
              stagger={0.025}
              className=\"text-3xl sm:text-4xl md:text-5xl lg:text-[3.2rem] font-black leading-[0.9] tracking-tight font-mono\"
            >
  ...
  [RP-∞]  IN PROGRESS
     └─ edge AI systems`}
        ...
        <div className=\"grid lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-10 lg:gap-14\">
          {/* Left: timeline-style list */}
          <div className=\"space-y-6 sm:space-y-7\">
            {publications.map((pub, index) => (
              <AnimatedSection key={pub.id} delay={index * 120}>
                <article className=\"group relative border-l-2 border-foreground/50 pl-5 sm:pl-7 py-5 sm:py-6\">
